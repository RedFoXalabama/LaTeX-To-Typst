...
- `/src/latex\_parser/latex math.pest`
...
- `arg\_text`: testo semplice di un `argument`
- `optional\_arg`: argomento opzionale del comando
- `optional\_list`: lista di argomenti che possono comporre un argomento opzionale
- `opt\_entry`: argomento singolo della lista
- `kv\_pair`: argomento chiave-valore
- `key`: chiave del `kv\_pair`
- `value`: valore del `kv\_pair`, può essere un semplice testo o una lista
- `simple\_value`: valore composto da testo semplice
- `value\_list`: valore composto da una lista di sottovalori
- `sub\_value\_list`: sottovalori della `value\_list`
- `opt\_item`: argomenti opzionali differenti dalla `kv\_pair`
- `opt\_text`: testo semplice dell' `opt\_item`
- `forbidden\_char`: insieme di caratteri da non considerare in determinate regole
...
Se la precedente struttura ad albero era basata sulla grammatica, questa nuova struttura ad albero che andiamo a realizzare è basata su una lista di strutture ed enumerativi da noi definiti, nel file _ast structure_, che vengono assegnati ai vari nodi in base alle loro regole grammaticali d'origine.
...
```
Gli attributi `optional args` e `required args` sono i vettori che contengono le rispettive liste di argomenti opzionali e obbligatori accettati dal comando, modellati da altre classi.
